app.controller('ListeSuiviController',function($scope,$http,apiUrl,CurrentUser,Notification,$window,$rootScope,$location){
    /***************************************************************************************************
     ***************************************** Start Controller ****************************************
     *************************************************************************************************** */
    /**
    ...
    new Inputmask("decimal", { rightAlign: false }).mask($('#monnaie'));
    /**
     * data list function
     * */
    function data_list(){
    ...
    $scope.Edit = function (suivi){
        if(suivi.ID_CLIENT != null){
            $location.path('SuiviCommercial/ModifSuiviEntreprise/'+suivi.ID_SUIVI);
        }else{
            $location.path('SuiviCommercial/ModifSuiviParticulier/'+suivi.ID_SUIVI);
        }
    ...
    $scope.Delete = function (id){
        swal({
            ...
            if (isConfirm) {
                $http({
                    method: 'DELETE',
                    url: apiUrl + 'FormaCampAPI/Suivi/'+id,
                    headers: {'Content-Type': undefined}
                }).then(function (response) {
                    if(response.data){
                        data_list();
