...
class LoginRequest extends FormRequest
{
    ...
    public function authenticate()
    {
        ...
        $acces = CompteAccesUsersModuleModel::leftjoin('acces_modules','acces_modules.ID_MODULES','acces_users.ID_MODULES')
            ->where('acces_users.ID_USER',$this->user()->ID_USER)
            ->get()
            ->pluck('TITRE')->toArray();
        $user = $this->user()->getAttributes();

        array_unshift($acces,$user['TYPE']);
        $user['TYPE'] = $acces;

        Cookie::queue('user', json_encode(Arr::except($user, ['password'])),null, null, null, false, false);
